...
export const createWork = async (req: Request, res: Response) => {
    ...
    try {
        const [result]: any = await db.query(
            'INSERT INTO Works (judul, deskripsi, kategori, klien, link_video, link_thumbnail, tahun_proyek) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [judul, deskripsi, kategori, klien, link_video, link_thumbnail, tahun_proyek]
        );
...
export const updateWork = async (req: Request, res: Response) => {
    ...
    try {
        await db.query(
            'UPDATE Works SET judul = ?, deskripsi = ?, kategori = ?, klien = ?, link_video = ?, link_thumbnail = ?, tahun_proyek = ? WHERE id = ?',
            [judul, deskripsi, kategori, klien, link_video, link_thumbnail, tahun_proyek, id]
        );
