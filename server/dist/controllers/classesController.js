...
export const createClass = async (req, res) => {
    ...
    try {
        const [result] = await db.query('INSERT INTO Classes (nama_kelas, deskripsi, harga, format, tanggal_mulai, jadwal_detail, kuota, status, link_thumbnail) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [nama_kelas, deskripsi, harga, format, tanggal_mulai, jadwal_detail, kuota, status, link_thumbnail]);
...
export const updateClass = async (req, res) => {
    ...
    const { nama_kelas, deskripsi, harga, format, tanggal_mulai, jadwal_detail, kuota, status, link_thumbnail } = req.body;
    try {
        await db.query('UPDATE Classes SET nama_kelas = ?, deskripsi = ?, harga = ?, format = ?, tanggal_mulai = ?, jadwal_detail = ?, kuota = ?, status = ?, link_thumbnail = ? WHERE id = ?', [nama_kelas, deskripsi, harga, format, tanggal_mulai, jadwal_detail, kuota, status, link_thumbnail, id]);
